<?php
function restbl_uststablebooking_install() {
	global $table_prefix, $wpdb;
  $table_name1 = $table_prefix.'restbl_uststablebookings';
  $sql_sfe = "CREATE TABLE IF NOT EXISTS ".$table_name1." (
                `tablebooking_id` int(11) NOT NULL AUTO_INCREMENT,
                ...
                `details` text,
                `tablebooking_by` varchar(255) DEFAULT NULL,
				`noof_seat` varchar(255) DEFAULT NULL,
                `confirmed` int(1) DEFAULT '0',
                `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
                PRIMARY KEY (`tablebooking_id`)
              ) ENGINE=InnoDB AUTO_INCREMENT=32 DEFAULT CHARSET=latin1;
              ";
   $wpdb->query($sql_sfe);
	 //------------------------------- 
	 $table_name2 = $table_prefix.'restbl_uststablebookings_paymentmethods';
   ...
   $wpdb->query($sql_scpm);
	 ...
	 $pm_count = $wpdb->get_results("SELECT COUNT(*) as pm_rowcount FROM ".$table_prefix."restbl_uststablebookings_paymentmethods");
   if(!$pm_count[0]->pm_rowcount){
      $sql_pm_data = "INSERT INTO `".$table_prefix."restbl_uststablebookings_paymentmethods` (`id`,`payment_method`) VALUES (2,'Bank'),(3,'Cash'),(4,'Paypal'),(5,'Credit Card'),(6,'Other');";
  ...
  if(!$curr_count[0]->rowcount){
    ...
    $wpdb->query($currency_tbl_data);
	 ...
	 //===============================End Currency Table==============================
	 //$table_shape_page_id = restbl_programmatically_create_page('Table Shapes','restbl-table-shapes','[restbl_tableshapes]','page');
   $table_shape_page_id = restbl_programmatically_create_page('Gen Tables','gen-restbl-tables','[gen_restbl_tables]','page');
   $uststablebooking_calendar_page_id = restbl_programmatically_create_page('Gen TableBooking Calendar','gen-usts-tablebooking-calendar','[gen_restbl_ustscalendar]','page');
}